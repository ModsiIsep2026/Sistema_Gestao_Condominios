(function () {
    ...
    const perfilLabel = { gestor: "Gestor", condomino: "Condómino", tecnico: "Técnico", admin: "Administrador" };
    ...
    async function carregarRecebidas() {
        try {
            ...
            if (!msgs.length) {
                tabelaRecebidas.innerHTML = `<tr><td colspan="5" class="app-vazio"><p>Nenhuma mensagem recebida.</p></td></tr>`;
                return;
            }
            ...
            tabelaRecebidas.querySelectorAll("tr[data-id]").forEach((tr) => {
                tr.addEventListener("click", async () => {
                    ...
                    const tipoRemetente = tr.dataset.tipoRemetente;

                    assuntoAtual        = tr.dataset.assunto;
                    idDestinatarioAtual = parseInt(tr.dataset.idRemetente);
                    ...
                    // Só mostrar zona de resposta para condóminos e técnicos (não para admin)
                    const zonaResposta = document.getElementById("zona-resposta");
                    zonaResposta.style.display  = tipoRemetente === "admin" ? "none" : "";
                    btnResponder.style.display  = tipoRemetente === "admin" ? "none" : "";
            ...
            tabelaRecebidas.innerHTML = `<tr><td colspan="5" class="app-vazio"><p>Erro ao carregar mensagens.</p></td></tr>`;
        }
    }
    ...
    async function carregarEnviadas() {
        try {
            ...
            if (!msgs.length) {
                tabelaEnviadas.innerHTML = `<tr><td colspan="3" class="app-vazio"><p>Nenhuma mensagem enviada.</p></td></tr>`;
                return;
            }
            ...
            tabelaEnviadas.querySelectorAll("tr[data-assunto]").forEach((tr) => {
                tr.addEventListener("click", () => {
                    ...
                    document.getElementById("resp-mensagem").value      = "";
                    erroResposta.style.display = "none";
                    document.getElementById("zona-resposta").style.display = "none";
    ...
    btnResponder.addEventListener("click", async () => {
        ...
        try {
            ...
            if (typeof window.mostrarToast === "function") window.mostrarToast("Resposta enviada.");
            carregarEnviadas();
        } catch (err) {
    ...
    document.getElementById("btn-enviar").addEventListener("click", async () => {
        ...
        try {
            ...
            carregarEnviadas();
            if (typeof window.mostrarToast === "function") window.mostrarToast("Mensagem enviada com sucesso.");
        } catch (err) {
