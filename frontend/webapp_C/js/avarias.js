...
(async function () {
    ...
    document.getElementById("btn-submeter-avaria")?.addEventListener("click", async () => {
        ...
        if (isNaN(id_edificio)) {
            if (erro) { erro.textContent = "Indique o edifício."; erro.hidden = false; }
            return;
        }
        if (!zona) {
            if (erro) { erro.textContent = "Indique a zona."; erro.hidden = false; }
            return;
        }
        ...
        try {
            ...
            if (erro) { erro.textContent = e.message || "Erro ao reportar."; erro.hidden = false; }
        } finally {
            btn.disabled    = false;
